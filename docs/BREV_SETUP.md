...
Проверено по пользовательским скриншотам: Ubuntu, Python 3.12.14, A100-SXM4-40GB, 40960 MiB видеопамяти, около 260 GiB свободного диска. PyTorch 2.8.0+cu128 видит CUDA и успешно выполняет умножение матриц. Пользователь скачал все три модели: `check_models.py` подтвердил локальные веса, затем остановился на загрузке pyannote с `UnpicklingError`. Полный прогон после обновления библиотеки ещё предстоит подтвердить.
