...
Статус: endpoints реализованы и проверены локальными тестами с заменой тяжёлых моделей тестовым обработчиком. Smoke check моделей на A100 пройден; качество на реальных записях и интеграция актуального frontend на Brev ещё требуют проверки. Адрес сервера и порядок установки интерфейса: [FRONTEND_INTEGRATION.md](FRONTEND_INTEGRATION.md). Владелец backend — лидер. Владелец frontend — второй участник. Изменения формата передавать друг другу до интеграции.
...
- Backend обслуживает сборку `frontend/dist`. Для клиентской навигации использовать HashRouter: fallback произвольных SPA-маршрутов на index.html пока отсутствует.
- Через HTTPS задаётся `HACKALEM_SECURE_COOKIE=1`. Вход `/login` устанавливает HttpOnly cookie; запросы, аудио и скачивание на том же origin используют её автоматически. Для прямого API допускается `Authorization: Bearer ...`. Код доступа к приложению отличается от токена Hugging Face.
...
|---|---|---|
| GET | `/api/health` | API и наличие скачанных весов; не заменяет пробный запуск моделей |
...
interface Speaker {
  ...
  evidence_segment_ids?: string[]; // источники предложенного имени; добавлено без смены существующих полей
}
...
Клиент отправляет только изменённые поля. Например, смена статуса — `{"status":"in_progress"}`; весь draft отправлять не следует. Отсутствующее поле означает «сохранить прежнее значение», а `due_date: null` — явную очистку даты. Это важно для сохранения исходного типа срока и причин проверки. При 401 необходимо предложить повторный вход через `/api/session`, сохранив несохранённый draft.
...
Предложения имён могут появиться уже на этапе `extract`, до готовности поручений. При `identification=suggested` показывать, что имя предложено ИИ и требует подтверждения. Таймаут извлечения возвращается как `failed` с `error.code=EXTRACTION_TIMEOUT`; уже сохранённые реплики и предложения имён доступны. Повторная обработка сохранённого транскрипта описана в [EXTRACTION_TROUBLESHOOTING.md](EXTRACTION_TROUBLESHOOTING.md).
